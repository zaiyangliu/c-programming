...
void testMaxSeq( int * array, size_t n, size_t correct_ans){
  if(maxSeq(array, n) < 0){
    printf("Bug1.\n");
    exit(EXIT_FAILURE);
  }
  if(maxSeq(array, n) != correct_ans){
    printf("Bug2.maxSeq(array, %zu)calculated to be %zu, expected answer is %zu.\n", n, maxSeq(array, n), correct_ans);
    exit(EXIT_FAILURE);
  }
  return;
}
